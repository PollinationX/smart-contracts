...
// PXUtils library for utility functions related to PollinationX
library PXUtils {
    ...
    function calculatePercentageDifference(uint256 oldValue, uint256 newValue) internal pure returns (uint256) {
        ...
        // Handle edge case where oldValue is zero to avoid division by zero
        if (oldValue == 0) {
            return 0;
        }
        // Calculate percentage difference and return
        uint256 percentageDifference = (difference * 100) / oldValue;
        return 100 - percentageDifference;
    }

    // Function to generate SVG progress image based on storageUnit, size, and usagePercentage
    function getProgressImage(
        string memory storageUnit,
        string memory size,
        string memory usagePercentage
    ) external pure returns (string memory) {
        // SVG image content encoded in bytes
        ...
        // Encode SVG to Base64 and return as a data URI
        return
            string(
            abi.encodePacked(
