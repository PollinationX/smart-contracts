...
contract PX is ERC721URIStorage, Ownable {
    ...
    function getProgressImage(uint256 tokenId, string memory storageUnit) public view returns (string memory) {
        bytes memory svg = abi.encodePacked(
            ...
            '.cls-9, .cls-10 {letter-spacing: -.03em;}',
    ...
    function updateStorageUsage(uint256 tokenId, uint256 usageInBytes) external onlyOwner() {
        ...
        pxStorage[tokenId].usagePercentage = PxHelper.calculatePercentageDifference(pxStorage[tokenId].sizeInBytes, newUsage);
    ...
    function upgradeTokenPackage(uint256 tokenId, uint256 packageId) public {
        require(_exists(tokenId), "Incorrect tokenId");
        require(_isApprovedOrOwner(_msgSender(), tokenId),"Not NFT owner");
        require(pxStoragePackage[packageId].sizeInBytes > pxStorage[tokenId].sizeInBytes, "You can only upgrade package");

        pxStorage[tokenId].usagePercentage = PxHelper.calculatePercentageDifference(pxStoragePackage[packageId].sizeInBytes, pxStorage[tokenId].usage);
